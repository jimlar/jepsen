    ...
    (setup! [_ test node]
      (c/cd "/tmp"
            (let [file "rabbitmq-server_3.2.4-1_all.deb"]
              ...
              (c/su
                ; Install package
                (try (c/exec :dpkg-query :-l :qpidd :corosync)
                     (catch RuntimeException _
                       (info node "installing QPid")
                       (c/exec :apt-get :install :-y :corosync :qpidd)))

                ; Set corosync configs
                (info node "setting corosync config")
                (c/exec :sed :-E :-i (str "s/bindnetaddr: [0-9.]+/bindnetaddr: 192.168.122.1" (node-num node) "/") "/etc/corosync/corosync.conf")
                (c/exec :echo "START=yes" :> "/etc/default/corosync")
                (c/exec :echo "uidgid { \nuid: qpid\ngid: qpid\n}" :> "/etc/corosync/uidgid.d/qpid")

                ; Ensure corosync is running
                (try (c/exec :service :corosync :status)
                     (catch RuntimeException _
                       (info node "starting corosync")
                       (c/exec :service :corosync :start)))

                ; Set qpid configs
                (info node "setting qpid config")
                (c/exec :sed :-E :-i "s/[#]*cluster-name=NAME/#cluster-name=jepsen-qpid/" "/etc/qpid/qpidd.conf")

                ; Ensure qpid is running
                (try (c/exec :service :qpidd :status)
                     (catch RuntimeException _
                       (info node "starting QPid")
                       (c/exec :service :qpidd :start)))
...
;        (meh (c/exec :service :rabbitmq-server :stop))
        (info node "Nuking Qpid")
        (meh (c/exec :killall :-9 "qpidd"))
;        (c/exec :rm :-rf "/var/lib/rabbitmq/mnesia/")
        (info node "Qpid dead, stopping corosync")
        (c/exec :service :corosync :stop),
        (info node "corosync stopped")))))
